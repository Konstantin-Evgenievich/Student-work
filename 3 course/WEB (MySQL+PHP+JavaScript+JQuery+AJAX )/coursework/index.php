    ...
    <center>
      ...
      <?php
        ...
        <?php while ($row = mysql_fetch_array($result)){?>
          ...
          <tr>
       <td><?=$row["id"]?></td><td><?=$row["floor"]?></td><td><?=$row["square"]?></td><td><?=$row["number_of_rooms"]?></td><td><?=$row["price"]?></td><td><?=$row["name_district"]?></td><td><?=$row["fullname"]?></td>
	<?php if(isset($_SESSION["user"])) {?>
<td><a href="update.php?id=<?=$row['id']?>&floor=<?=$row['floor']?>&square=<?=$row['square']?>&number_of_rooms=<?=$row['number_of_rooms']?>&price=<?=$row['price']?>&district_id=<?=$row['name_district']?>&owner_id=<?=$row['fullname']?>">
 ...
 <form method = "post" action="index.php">
  ...
  <a href = 'admin/index.php'>Admin (админ)</a> <br>
  <?php }?>
  <a href = exit.php> LOGOUT (выход)</a><br>
  <?php }
  else {?>
  <a href = login.php>LOGIN (войти)</a> <br>
  <?php
  }?>
</div>
</body>
